    ...
    try {
        ...
        spread.import(file, function () {
            ...
            rid2base64.forEach((base64, rid) => {
                ...
                let formula = GC.Spread.Sheets.CalcEngine.rangeToFormula(new GC.Spread.Sheets.Range(row, 0, 1, 1))
...
async function buildImageIdMap(zip) {
    ...
    zip.forEach((relativePath, file) => {
        if ((relativePath.startsWith('xl/drawings/_rels/') && relativePath.endsWith('.rels')) || (relativePath.startsWith('xl/_rels/') && relativePath.endsWith('cellimages.xml.rels'))) {
            relFiles.push(file);
        }
    });
