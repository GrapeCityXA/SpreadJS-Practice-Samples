...
function eventCallback(sheet, selection, els) {
    ...
    let selRB_Rect = sheet.getCellRect(selection.row + selection.rowCount - 1, selection.col + selection.colCount - 1)
    ...
    if (!selRect.x || !selRect.y || !selRect.width || !selRect.height) {
        ...
        // 选择单元格在可视区域【上方】
        if (selection.row < topRow && selection.col >= leftCol && selection.col + selection.colCount - 1 <= rightCol) {
            selRect.x = sheet.getCellRect(topRow, selection.col).x
            setElPosition(canvasRect, selRect, els, "bottom") // 只需要一个bottom
        }
        // 选择单元格在可视区域【下方】
        else if (selection.row > bottomRow && selection.col >= leftCol && selection.col + selection.colCount - 1 <= rightCol) {
            selRect.x = sheet.getCellRect(bottomRow, selection.col).x
            setElPosition(canvasRect, selRect, els, "top") // 只需要一个top
        }
        // 选择单元格在可视区域【左方】
        else if (selection.col < leftCol && selection.row >= topRow && selection.row + selection.rowCount - 1 <= bottomRow) {
            selRect.y = sheet.getCellRect(selection.row, leftCol).y
            setElPosition(canvasRect, selRect, els, "right") // 只需要一个right
        }
        // 选择单元格在可视区域【右方】
        else if (selection.col > rightCol && selection.row >= topRow && selection.row + selection.rowCount - 1 <= bottomRow) {
